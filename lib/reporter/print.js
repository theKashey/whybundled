...
const printReasons = (
  ...
  const tree = reasonsSubSet.reduce((acc, reason /*: Reason */) => {
    if (reason.type === "file") {
      acc[
        `${mapping[reason.moduleName]}|${printReasonName(
          reason.moduleName,
          by
        )}  ${chalk.dim(reason.loc)}  ${chalk.dim(
          "[" + reason.importType + "]"
        )}`
      ] = null;
...
const printFile = (module /*: Module */, chunks, chunkMapping, limit, by) => {
...
const printModule = (module /*: Module */, chunks, chunkMapping, limit, by) => {
  const chunksInfo = printChunkInfo(module, chunks);
  const entry = isEntry(module.reasons);
  return [
